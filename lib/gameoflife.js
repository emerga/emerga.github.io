...
/******/ 	return __webpack_require__(__webpack_require__.s = "./src/scripts/game-of-life/index.js");
/******/ })
/************************************************************************/
/******/ ({
...
/***/ "./src/lib/conway-state-machine.js":
/*!*****************************************!*\
  !*** ./src/lib/conway-state-machine.js ***!
  \*****************************************/
/*! no static exports found */
...
var Cell = /*#__PURE__*/function (_state$EntityElement) {
  _inherits(Cell, _state$EntityElement);

  var _super = _createSuper(Cell);

  function Cell(machine, state, callback) {
  ...
  _createClass(Cell, [{
    ...
    get: function get() {
      ...
      var cellStateInt = function cellStateInt(bx, by) {
      ...
      var x = this.x,
          y = this.y;
      return cellStateInt(x - 1, y - 1) + cellStateInt(x - 1, y) + cellStateInt(x - 1, y + 1) + cellStateInt(x, y - 1) + cellStateInt(x, y + 1) + cellStateInt(x + 1, y - 1) + cellStateInt(x + 1, y) + cellStateInt(x + 1, y + 1);
    }
  }], [{
    key: "cellId",
...
var StateMachineWrappingGridCollection = /*#__PURE__*/function (_state$StateMachineCo) {
  _inherits(StateMachineWrappingGridCollection, _state$StateMachineCo);

  var _super2 = _createSuper(StateMachineWrappingGridCollection);

  function StateMachineWrappingGridCollection(x, y, machine) {
    var _this2;

    _classCallCheck(this, StateMachineWrappingGridCollection);

    _this2 = _super2.call(this, machine);
    _this2._x = x;
    _this2._y = y;
  ...
  _createClass(StateMachineWrappingGridCollection, [{
    key: "add",
    value: function add(x, y, el) {
  ...
  return StateMachineWrappingGridCollection;
}(state.StateMachineCollection);

var ConwayStateMachineRules = /*#__PURE__*/function () {
  function ConwayStateMachineRules(app) {
    _classCallCheck(this, ConwayStateMachineRules);
  ...
  _createClass(ConwayStateMachineRules, [{
    key: "evaluate",
    value: function evaluate(state, neighbors) {
  ...
  return ConwayStateMachineRules;
}();

var ConwayStateMachine = /*#__PURE__*/function (_state$StateMachine) {
  _inherits(ConwayStateMachine, _state$StateMachine);

  var _super3 = _createSuper(ConwayStateMachine);

  /**
   * 
   * @param {*} emerga 
   */
  function ConwayStateMachine(width, height, cellSize, percentAlive) {
    var _this3;

    _classCallCheck(this, ConwayStateMachine);

    _this3 = _super3.call(this);
    singleton = _assertThisInitialized(_this3);
    _this3._cellSize = cellSize;
    _this3._gridWidth = width;
    _this3._gridHeight = height;
    _this3._percentAlive = percentAlive;
    _this3._onEmitElement = null;
    _this3._onEmitIteration = null;
    _this3._rules = new ConwayStateMachineRules(_assertThisInitialized(_this3));
    _this3.gridNumbers = [];
    _this3._gridHeight = height;
    _this3.elements = new StateMachineWrappingGridCollection(width, height, _assertThisInitialized(_this3));

    var machine = _assertThisInitialized(_this3);

    _this3.builders.add(new state.StateBuilder(_assertThisInitialized(_this3), null, function () {
      for (var x = 0; x < machine.gridWidth; x++) {
        for (var y = 0; y < machine.gridHeight; y++) {
          var cellCnt = machine.gridWidth * machine.gridHeight;
          var nel = new Cell(machine, {
            id: Cell.cellId(x, y),
            x: x,
            y: y,
            state: Math.random() * cellCnt < cellCnt / machine.percentAlive,
            updated: true
          }, Cell.advance);
          machine.elements.add(x, y, nel);
        }
      }
    }));

    _this3.operators.add(new state.StateOperator(_assertThisInitialized(_this3), null, function (el, iter) {
      return el.execute(iter);
    }));

    _this3.postIteration = function (machine, els) {
      var gn = 0;
    ...
    machine.renderers.add(new state.StateRenderer(machine, null, function (el) {
      return machine.doEmitElement(el);
    }));
    return _this3;
  }

  _createClass(ConwayStateMachine, [{
    key: "rulesUpdated",
    value: function rulesUpdated() {
      singleton.stop();
      setTimeout(function () {
        singleton.run(-1);
      });
    ...
    key: "gridNumber",
    get: function get() {
      return this.gridNumbers.reduce(function (a, b) {
        return a + b;
      }, 0) / this.gridNumbers.length;
    },
    set: function set(v) {
    ...
    get: function get() {
      ...
      return roc.reduce(function (a, b) {
        return a + b;
      }, 0) / roc.length;
    }
  }, {
  ...
  return ConwayStateMachine;
}(state.StateMachine);

module.exports = {
  ConwayStateMachine: ConwayStateMachine,
  StateMachineWrappingGridCollection: StateMachineWrappingGridCollection,
  Cell: Cell
};
...
var GameOfLife = /*#__PURE__*/function () {
  ...
  _createClass(GameOfLife, [{
    ...
    value: function setupStateMachine() {
      this.stateMachine = new conway.ConwayStateMachine(120, 80, this._cellSize, this._percentAlive);
      this.stateMachine.onEmitIteration = this.iterationEmitted;
    }
    /**
     * 
     */
...
/***/ "./src/scripts/game-of-life/ui.js":
/*!****************************************!*\
  !*** ./src/scripts/game-of-life/ui.js ***!
  \****************************************/
/*! exports provided: initUI, initCanvas, animate, addToRender */
/***/ (function(module, __webpack_exports__, __webpack_require__) {
...
var clearCanvas = function clearCanvas(app) {
  app.emerga.context.fillColor = 'rgba(0,0,0,1)';
...
var initControls = function initControls(app) {
  ...
  folder.add(app, 'percentAlive', 1, 99).step(1).onChange(app.settingsUpdated());
  folder.open();
  var rulesObj = app.stateMachine.rules;
  var fr = folder.addFolder('Game of life Rules');
  var fonr = fr.addFolder('On State Rules');
  var foffr = fr.addFolder('Off State Rules');
  fonr.add(rulesObj, 'On1').onChange(app.settingsUpdated());
  fonr.add(rulesObj, 'On2').onChange(app.settingsUpdated());
  fonr.add(rulesObj, 'On3').onChange(app.settingsUpdated());
  fonr.add(rulesObj, 'On4').onChange(app.settingsUpdated());
  fonr.add(rulesObj, 'On5').onChange(app.settingsUpdated());
  fonr.add(rulesObj, 'On6').onChange(app.settingsUpdated());
  fonr.add(rulesObj, 'On7').onChange(app.settingsUpdated());
  fonr.add(rulesObj, 'On8').onChange(app.settingsUpdated());
  foffr.add(rulesObj, 'Off1').onChange(app.settingsUpdated());
  foffr.add(rulesObj, 'Off2').onChange(app.settingsUpdated());
  foffr.add(rulesObj, 'Off3').onChange(app.settingsUpdated());
  foffr.add(rulesObj, 'Off4').onChange(app.settingsUpdated());
  foffr.add(rulesObj, 'Off5').onChange(app.settingsUpdated());
  foffr.add(rulesObj, 'Off6').onChange(app.settingsUpdated());
...
var renderElement = function renderElement(app, el) {
  ...
  ctx.strokeRect(x, y, app.cellSize, app.cellSize);
};
/**
 * 
 */
