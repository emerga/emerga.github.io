...
var FractalOfLife = /*#__PURE__*/function () {
  ...
  _createClass(FractalOfLife, [{
    ...
    value: function init() {
      ...
      g.add(self, 'segments').min(2).step(1).onChange(function (val) {
        self.segments = val; //stopStateMachine(startStateMachine);
      });
      g.add(self, 'numIterations').min(1).step(1).onChange(function (val) {
        self.numIterations = val; //stopStateMachine(startStateMachine);
      });
      g.add(self, 'branchAngle', 1, 359).onChange(function (val) {
        self.branchAngle = val; //stopStateMachine(startStateMachine);
      });
      g.add(self, 'scaleFactor', 1, 8192).onChange(function (val) {
        self.scaleFactor = val; //stopStateMachine(startStateMachine);
      });
      g.remember(self);
      ...
      machine.renderers.add(new state.StateRenderer(machine, null, function (el, iter) {
        ...
        var strokeStyle = fullColor(getRgb(iter), iter > 6 ? 1 / 6 : 1 / iter);
        var strokeWidth = iter > 6 ? 1 : 6 / iter;
