  ...
  _createClass(StateMachine, [{
    ...
    value: function advance(iterations) {
      ...
      var _advance = function _advance() {
        newElements = [];
        curElements = self.elements.elements;
        self.elements.elements = [];

        if (self.remIterations === 0 || self.halt || curElements.length === 0) {
          return;
        }

        var operateOnElement = function operateOnElement(stateEl, done) {
          //setTimeout(()=>{
          var els = stateEl.execute(self.iteration);
          done(null, els); //}, 0);  
        };

        var renderElement = function renderElement(stateEl, done) {
          setTimeout(function () {
            self.renderers.elements[0].execute(self.iteration, stateEl);
            if (done) done(null);
          }, 0);
        };
        ...
        var processElement = function processElement() {
          if (curElements.length === 0) {
            self.postIteration(self, newElements, self.iteration);
            self.iteration++;
            self.remIterations--;
            return setTimeout(_advance, 0);
          }

          var curElement = curElements.pop();
          operateOnElement(curElement, function (err, els) {
            renderElement(curElement);
            newElements = newElements.concat(els);
            return setTimeout(processElement, 0);
          });
        };
...
var BloomElement = __webpack_require__(/*! ../../lib/bloom-element */ "./src/lib/bloom-element.js").BloomElement;
...
var ui = __webpack_require__(/*! ./ui */ "./src/scripts/fractal-of-life/ui.js");

var initInterface = ui.initInterface;
var initToolbar = ui.initToolbar;
var singleton;
...
var FractalOfLife = /*#__PURE__*/function () {
  ...
  function FractalOfLife(emerga) {
    ...
    this._lineBrightness = 50;
    this._twistFactor = 0;
    this._segmentStartIndex = 0;
    this._globalCompositeOperation = 'luminosity';
    this._elementsToRender = [];
  }

  _createClass(FractalOfLife, [{
    key: "popElementForRendering",
    value: function popElementForRendering() {
      return this.elementsToRender.pop();
    }
  }, {
    key: "pushElementForRendering",
    value: function pushElementForRendering(el) {
      this.elementsToRender.push(el);
    }
    /**
     * 
     */
    ...
    value: function settingsUpdated() {
      if (!singleton.inited) return; // stop the state machine

      var stopStateMachine = function stopStateMachine(cb) {
        singleton.stateMachine.stop(cb);
      };

      if (singleton.stateMachine && !singleton.stateMachine.halted) stopStateMachine(singleton.startStateMachine);
    }
    /**
     * 
     */
    ...
    key: "initStateMachine",
    value: function initStateMachine() {
      var self = this;
    ...
    value: function renderElement(el) {
      var self = singleton;
      var iter = singleton.stateMachine.iteration;
      var strokeWidth = iter > 8 ? 1 : 8 / iter;
      self.context.globalCompositeOperation = self.globalCompositeOperation;

      var drawLine = function drawLine(start, end) {
        var strokeStyle = fullColor(getRgb(iter), 0.01 * self.lineBrightness);
        ...
        if (!self.useFabric) {
          if (self.useLines) {
            ...
            self.context.lineWidth = self.lineThickness;
            self.context.strokeWidth = self.variableLineThickness ? self.lineThickness * strokeWidth / 2 : self.lineThickness;
          }

          if (self.usePoints) {
            var pointSize = self.variablePointSize ? self.scaleFactor / (iter * self.pointScale) : self.pointScale;
            self.context.arc(start[0], start[1], pointSize, 0, 2 * Math.PI, false);
    ...
    value: function startStateMachine() {
      var curCanv = document.getElementById('main');
      if (curCanv) curCanv.parentNode.removeChild(curCanv);
      var canv = document.createElement('canvas');
      canv.id = 'main';
      canv.width = '1200';
      canv.height = '1200';
      document.body.appendChild(canv);
      singleton.initCanvas();
      singleton.initStateMachine();
      singleton.stateMachine.run(singleton.numIterations);
    }
    /**
     * 
     */
    ...
    value: function animate() {
      var _animate = function _animate() {
        singleton.emerga.stats[0].begin();
        var el = singleton.popElementForRendering();
        if (el) singleton.renderElement(el);
        singleton.emerga.stats[0].end();
    ...
    key: "elementsToRender",
    get: function get() {
      return this._elementsToRender;
    }
  }]);
...
/*!*******************************************!*\
  !*** ./src/scripts/fractal-of-life/ui.js ***!
  \*******************************************/
/*! exports provided: initInterface, initToolbar */
/***/ (function(module, __webpack_exports__, __webpack_require__) {
...
var initInterface = function initInterface(app) {
  g = app.emerga.guis['gui'];
  var folder = g.addFolder('Starting Conditions');
  folder.add(app, 'segments').min(2).step(1).onChange(app.settingsUpdated());
  ...
  folder.add(app, 'lineBrightness').onChange(app.settingsUpdated());
...
var ui = __webpack_require__(/*! ./ui */ "./src/scripts/wolfram-model/ui.js");

var WolframModel = /*#__PURE__*/function () {
  /**
   * 
   * @param {*} emerga 
   */
  function WolframModel(emerga) {
    _classCallCheck(this, WolframModel);

    var self = this;
    this.emerga = emerga;
    this.inited = false;
    emerga.initCanvas('main');
    this._globalCompositeOperation = 'luminosity';
  }

  _createClass(WolframModel, [{
    key: "settingsUpdated",
    ...
    value: function initInterface() {
      //this.emerga.canvas.style.visibility = "hidden";
      this.emerga.initStats(0);
      ui.initInterface(this);
    }
    /**
     * 
     */
...
module.exports = WolframModel;

/***/ }),

/***/ "./src/scripts/wolfram-model/ui.js":
/*!*****************************************!*\
  !*** ./src/scripts/wolfram-model/ui.js ***!
  \*****************************************/
/*! exports provided: initInterface */
/***/ (function(module, __webpack_exports__, __webpack_require__) {
...
var initControls = function initControls(app) {
  app.emerga.initGUI('gui', {
    load: JSON,
    preset: WolframModel
  });
  self.g = app.emerga.guis['gui'];
  var folder = g.addFolder('Starting Conditions');
  folder.open();
  folder = g.addFolder('Rendering');
  folder.add(app, 'globalCompositeOperation', ['source-over', 'source-in', 'source-out', 'source-stop', 'destination-over', 'destination-in', 'destination-out', 'destination-stop', 'lighter', 'copy', 'xor', 'multiply', 'screen', 'overlay', 'darken', 'lighten', 'color-dodge', 'color-burn', 'hard-light', 'difference', 'exclusion', 'hue', 'saturation', 'color', 'luminosity']).onChange(app.settingsUpdated());
...
/******/ });
//# sourceMappingURL=emergasamples.js.map