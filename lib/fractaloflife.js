...
/******/ 	return __webpack_require__(__webpack_require__.s = "./src/scripts/fractal-of-life.js");
/******/ })
/************************************************************************/
/******/ ({
      ...
      var _advance = function _advance() {
        ...
        var operateOnElement = function operateOnElement(stateEl, done) {
          setTimeout(function () {
            ...
            var els = [];
            each(self.operators.elements, function (stateOp, cb) {
              var childrenEls = stateOp.execute(self.iteration, stateEl, childrenEls);
              els = els.concat(childrenEls);
              cb();
            }, function (err) {
              if (err) {
                return done(err);
        ...
        var renderElement = function renderElement(stateEl, done) {
          setTimeout(function () {
            ...
            each(self.renderers.elements, function (stateRe, cb) {
        ...
        each(self.elements.elements, function (stateEl, done) {
          operateOnElement(stateEl, function (err, els) {
            renderElement(stateEl);
            each(els, function (ell, cb) {
              renderElement(ell);
...
var FractalOfLife = /*#__PURE__*/function () {
  function FractalOfLife(emerga) {
    ...
    this.numIterations = this._numIterations = 6;
    this.scaleFactor = this._scaleFactor = 256;
    this.offset = 600;
    this.useFabric = false;
    this.resetting = false;
    this.usePoints = this._usePoints = true;
    this.useLines = this._useLines = true;
    this.pointScale = this._pointScale = 16;
    this.variablePointSize = this._variablePointSize = true;
    this.variableLineThickness = this._variableLineThickness = true;
    this.lineThickness = this._lineThickness = 10;
    this.lineBrightness = this._lineBrightness = 100;
    this.globalCompositeOperation = this._globalCompositeOperation = 'luminosity';
    this.initCanvas('main');
  }
  ...
  _createClass(FractalOfLife, [{
    ...
    value: function init() {
      ...
      var self = this; // stop the state machine

      var stopStateMachine = function stopStateMachine(cb) {
        self.stateMachine.stop();
        self.stateMachine.reset(function () {
          self.resetting = true;
          if (cb) cb();
        });
      };
      ...
      var startStateMachine = function startStateMachine() {
        ...
        var width = curCanv.width,
            height = curCanv.height;
        ...
        initStateMachine();
        self.stateMachine.run(self.numIterations);
      };
      ...
      var _animate = function _animate() {
        self.emerga.stats[0].begin();
        self.emerga.stats[0].end(); //window.requestAnimationFrame(_animate);
      };

      var initEmerga = function initEmerga() {
        _this.emerga.initStats(0).initGUI('gui', {
          load: JSON,
      ...
      var initInterface = function initInterface() {
        var g = _this.emerga.guis['gui'];
        var folder = g.addFolder('Starting Conditions');
        folder.add(self, '_segments').min(2).step(1).onChange(function (val) {
          if (val !== self.segments) {
            self.segments = val;
            stopStateMachine(startStateMachine);
            console.log('_segments', val);
          }
        });
        folder.add(self, '_numIterations').min(1).step(1).onChange(function (val) {
          if (val !== self.numIterations) {
            self.numIterations = val;
            stopStateMachine(startStateMachine);
            console.log('_numIterations', val);
          }
        });
        folder.add(self, '_branchAngle', 1, 359).onChange(function (val) {
          if (val !== self.branchAngle) {
            self.branchAngle = val;
            stopStateMachine(startStateMachine);
            console.log('_branchAngle', val);
          }
        });
        folder.open();
        folder = g.addFolder('Scaling');
        folder.add(self, '_scaleFactor', 1, 8192).onChange(function (val) {
          if (val !== self.scaleFactor) {
            self.scaleFactor = val;
            stopStateMachine(startStateMachine);
            console.log('_scaleFactor', val);
          }
        });
        folder.add(self, '_pointScale').onChange(function (val) {
          self.pointScale = val;
          stopStateMachine(startStateMachine);
          console.log('_pointScale', val);
        });
        folder.open();
        folder = g.addFolder('Rendering');
        folder.add(self, '_usePoints').onChange(function (val) {
          if (val !== self.usePoints) {
            self.usePoints = val;
            stopStateMachine(startStateMachine);
            console.log('_usePoints', val);
          }
        });
        folder.add(self, '_useLines').onChange(function (val) {
          if (val !== self.useLines) {
            self.useLines = val;
            stopStateMachine(startStateMachine);
            console.log('_useLines', val);
          }
        });
        folder.add(self, '_variablePointSize').onChange(function (val) {
          if (val !== self.variablePointSize) {
            self.variablePointSize = val;
            stopStateMachine(startStateMachine);
            console.log('_variablePointSize', val);
          }
        });
        folder.add(self, '_variableLineThickness').onChange(function (val) {
          if (val !== self.variableLineThickness) {
            self.variableLineThickness = val;
            stopStateMachine(startStateMachine);
            console.log('_variableLineThickness', val);
          }
        });
        folder.add(self, '_lineThickness').onChange(function (val) {
          if (val !== self.lineThickness) {
            self.lineThickness = val;
            stopStateMachine(startStateMachine);
            console.log('_lineThickness', val);
          }
        });
        folder.add(self, '_lineBrightness').onChange(function (val) {
          if (val !== self.lineBrightness) {
            self.lineBrightness = val;
            stopStateMachine(startStateMachine);
            console.log('_lineBrightness', val);
          }
        });
        folder.add(self, '_globalCompositeOperation', ['source-over', 'source-in', 'source-out', 'source-stop', 'destination-over', 'destination-in', 'destination-out', 'destination-stop', 'lighter', 'copy', 'xor', 'multiply', 'screen', 'overlay', 'darken', 'lighten', 'color-dodge', 'color-burn', 'hard-light', 'difference', 'exclusion', 'hue', 'saturation', 'color', 'luminosity']).onChange(function (val) {
          if (val !== self.globalCompositeOperation) {
            self.globalCompositeOperation = val;
            stopStateMachine(startStateMachine);
            console.log('globalCompositeOperation', val);
          }
        });
        folder.open();
        g.remember(self);
      ...
      var initStateMachine = function initStateMachine() {
        ...
        machine.postIteration = function (machine, _els, iter) {
          machine.elements.elements.forEach(function (mel) {
            mel.state.parent = mel;
          });
        ...
        machine.renderers.add(new state.StateRenderer(machine, null, function (el, iter) {
          ...
          if (!self.useFabric && !el.state.drawn) self.context.stroke();
        }));
      };
