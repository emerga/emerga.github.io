...
var FractalOfLife = /*#__PURE__*/function () {
  function FractalOfLife(emerga) {
    ...
    this.lineBrightness = this._lineBrightness = 1;
    this.globalCompositeOperation = this._globalCompositeOperation = 'multiply';
  ...
  _createClass(FractalOfLife, [{
    ...
    value: function init() {
      ...
      var initInterface = function initInterface() {
        ...
        g.add(self, '_usePoints').onChange(function (val) {
        ...
        g.add(self, '_useLines').onChange(function (val) {
        ...
        g.add(self, '_pointScale').onChange(function (val) {
          if (val !== self.pointScale) {
            self.pointScale = val;
            stopStateMachine(startStateMachine);
            console.log('_pointScale', val);
          }
        });
        g.add(self, '_lineThickness').onChange(function (val) {
        ...
        g.add(self, '_lineBrightness').onChange(function (val) {
      ...
      var initStateMachine = function initStateMachine() {
        ...
        machine.renderers.add(new state.StateRenderer(machine, null, function (el, iter) {
          ...
          var strokeStyle = fullColor(getRgb(iter), 0.01);
          ...
          var drawLine = function drawLine(start, end) {
            if (!self.useFabric) {
              if (self.useLines) {
                ...
                self.context.strokeStyle = fullColor(getRgb(iter), 0.01 * self.lineBrightness);
                self.context.strokeWidth = self.lineThickness * strokeWidth / 2;
              }

              if (self.usePoints) {
                self.context.arc(start[0], start[1], self.scaleFactor / (iter * 8), 0, 2 * Math.PI, false);
          ...
          if (self.resetting) {
            ...
            clearCanvas(); // self.canvas.width = self.canvas.width;
          }
