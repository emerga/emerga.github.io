  ...
  _createClass(StateMachine, [{
    ...
    value: function advance(iterations) {
      ...
      var advance = function advance() {
        _this2.remIterations = iterations;

        if (!(_this2.remIterations >= 0 && _this2.iteration < _this2.maxIterations)) {
          return;
        }

        var newElements = [];

        _this2.elements.elements.forEach(function (stateEl) {
          var els = [];
          ...
          els.forEach(function (ell) {
            setTimeout(function () {
              self.renderers.elements.forEach(function (stateRe) {
                stateRe.execute(self.iteration, ell);
              });
            }, 0);
        ...
        if (_this2.postIteration) {
          _this2.postIteration(_this2, newElements);
        }

        _this2.iteration++;
        _this2.remIterations = _this2.remIterations !== -1 ? _this2.remIterations - 1 : -1;
  ...
  _createClass(BloomElement, [{
    ...
    value: function bloom(el, iter) {
      ...
      for (var i = 0; i < el.segments; i++) {
        ...
        var angle = 27 * (1 / 2 + i - 5 / 2) + el.angle;
  ...
  _createClass(FractalOfLife, [{
    ...
    value: function init() {
      ...
      machine.renderers.add(new state.StateRenderer(machine, null, function (el, iter) {
        ...
        var originCoords = offset(el.origin(2048), 3200);

        if (el.parent) {
          var parentCoords = offset(el.parent.origin(2048), 3200);
          canvasContext.beginPath();
          var rgbVal = getRgb((iter - 1) * 10);
          canvasContext.strokeColor = fullColorHex(rgbVal);
          canvasContext.fillColor = fullColorHex(rgbVal);
          canvasContext.moveTo(originCoords[0], originCoords[1]);
          canvasContext.lineTo(parentCoords[0], parentCoords[1]);
          canvasContext.stroke();
        }

        el.children.forEach(function (child) {
          var destCoords = offset(child.origin(2048), 3200);
          canvasContext.beginPath();
          var rgbVal = getRgb(iter * 10);
          canvasContext.strokeColor = fullColorHex(rgbVal);
          canvasContext.fillColor = fullColorHex(rgbVal);
          canvasContext.moveTo(originCoords[0], originCoords[1]);
